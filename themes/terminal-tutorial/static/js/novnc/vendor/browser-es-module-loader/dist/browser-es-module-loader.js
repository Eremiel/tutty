...
// sanitize out the hash and querystring
  if (baseURI) {
    ...
    const slashIndex = baseURI.lastIndexOf('/');
 ...
 */
  ...
  function RegisterLoader$1() {
    ...
    const registryDelete = this.registry.delete;
    this.registry.delete = function (key) {
      let deleted = registryDelete.call(this, key);
  ...
  const INSTANTIATE = RegisterLoader$1.instantiate = createSymbol('instantiate');
...
// for dynamic modules, the load record is removed entirely.
  ...
  RegisterLoader$1.prototype[Loader.resolveInstantiate] = function (key, parentKey) {
    ...
    const registry = this.registry[REGISTRY];
  ...
  .then(instantiated => {
    ...
    const link = instantiated.linkRecord;
    ...
    .then(() => ensureEvaluate(loader, instantiated, link, registry, state, undefined));
  });
  };
 ...
 */
  function registerDeclarative(loader, load, link, declare) {
    ...
    let definedExports = false;

  // closure especially not based on link to allow link record disposal
    const declared = declare.call(envGlobal, (name, value) => {
      ...
      for (let i = 0; i < importerSetters.length; i++) { importerSetters[i](moduleObj); }
  ...
  function instantiateDeps(loader, load, link, registry, state) {
    ...
    for (let i = 0; i < link.dependencies.length; i++) { depsInstantiatePromises[i] = resolveInstantiateDep(loader, link.dependencies[i], load.key, registry, state, loader.trace && link.depMap || (link.depMap = {})); }

    let depsInstantiatePromise = Promise.all(depsInstantiatePromises)
  .then(dependencyInstantiations => {
    ...
    if (loader.trace) {
      depsInstantiatePromise = depsInstantiatePromise.then(() => {
        traceLoad(loader, load, link);
        return load;
      });
    }

    depsInstantiatePromise = depsInstantiatePromise.catch(err => {
    // throw up the instantiateDeps stack
      link.depsInstantiatePromise = undefined;
      throw LoaderError__Check_error_message_for_loader_stack(err, `Loading ${load.key}`);
    });

    depsInstantiatePromise.catch(() => {});
      ...
      function processLoad(load) {
        ...
        if (link) {
          for (let i = 0; i < link.dependencies.length; i++) {
            ...
            if (!(depLoad instanceof ModuleNamespace)) { queueLoad(depLoad); }
          }
        }
...
// returns the error if any
    ...
    if (link.setters) {
      ...
      // custom Module returned from instantiate
        ...
        if (depLink && seen.indexOf(depLoad) === -1) {
          if (depLoad.evalError) { err = depLoad.evalError; } else
  ...
  // dispose link record
    ...
    if (err) { return load.evalError = LoaderError__Check_error_message_for_loader_stack(err, `Evaluating ${load.key}`); }
...
// {} is the closest we can get to call(undefined)
  ...
  function dynamicExecute(execute, require, exports, module) {
    try {
      const output = execute.call(envGlobal, require, exports, module);
...
// normalize is never given a relative name like "./x", that part is already handled
  BrowserESModuleLoader.prototype[RegisterLoader$1.resolve] = function (key, parent) {
    const resolved = RegisterLoader$1.prototype[RegisterLoader$1.resolve].call(this, key, parent || this.baseKey) || key;
